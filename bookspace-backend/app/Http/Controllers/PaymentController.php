...
class PaymentController extends Controller
{
    ...
    private function notifySuccessfulPayment(Order $order): void
    {
        ...
        $notifications->sendOnce(
            $order->id_client,
            'paiement_confirme',
            'Le paiement de votre commande a �t� confirm�.',
            $order->id_commande
        );
        ...
        foreach ($order->items as $item) {
            ...
            if ($sellerClientId) {
                $notifications->sendOnce(
                    ...
                    "Une commande pay�e contient � {$title} �. Pr�parez cette ligne de commande.",
            ...
            if ($item->offer?->type_offre === 'numerique') {
                $notifications->sendOnce(
                    $order->id_client,
                    'ebook_disponible',
                    "Votre e-book � {$title} � est disponible dans votre biblioth�que num�rique.",
                    $item->id_ligne_commande
                );
            }
        }
    ...
    public function mobileMoneyStatus(Request $request, Order $order, string $reference)
    {
        ...
        $paymentExists = Payment::query()
            ...
            ->exists();
        abort_unless($paymentExists, 404, 'Cette référence ne correspond pas à un paiement de cette commande.');
