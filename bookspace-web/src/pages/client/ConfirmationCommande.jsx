...
import { CheckCircle2, BookOpen, MapPin, Smartphone } from "lucide-react";
...
export default function ConfirmationCommande() {
  ...
  const [articles] = useState(() => getCartItems());
    ...
    if (orderId) {
      fetchOrderById(orderId)
        .then((order) => {
          ...
          if (["paid", "shipped", "delivered"].includes(order.status)) clearCart();
        })
        .catch(() => {});
    } else if (commande && ["paid", "shipped", "delivered"].includes(commande.status)) {
      clearCart();
    }
  }, [location.search, commande?.status]);
    ...
    <div className="px-4 sm:px-6 lg:px-10 py-8">
      <div className="max-w-[900px] mx-auto">
        <div className="card !bg-success-bg !border-none flex flex-col sm:flex-row items-center gap-4 mb-6 text-center sm:text-left">
          <CheckCircle2 size={40} className={`${statutPaiement === "paid" ? "text-success" : "text-warning"} shrink-0`} />
          <div className="flex-1">
            <div className="font-head text-xl font-extrabold">{statutPaiement === "paid" ? "Paiement confirmé" : "Commande créée"}</div>
            <div className="text-muted text-sm">Statut du paiement : {statutPaiement === "paid" ? "Payé" : statutPaiement === "pending" ? "En attente de confirmation" : statutPaiement}</div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6 text-xs">
          ...
          <div className="card !p-3.5"><div className="text-faint mb-1">Date</div><b>{commande?.created_at ? new Date(commande.created_at).toLocaleDateString("fr-FR") : "—"}</b></div>
          <div className="card !p-3.5"><div className="text-faint mb-1">Montant</div><b>{commande?.total_amount ?? "—"} XAF</b></div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-6">
          {papier.length > 0 && (
            <div className="card">
              <div className="flex items-center justify-between mb-3">
                <div className="font-bold text-sm flex items-center gap-2"><MapPin size={16} /> Articles physiques</div>
              </div>
              {papier.map((a) => (
              ...
              <div className="bg-surfaceAlt rounded-lg p-3.5 mt-3 flex items-center gap-3">
                ...
                <div>
                  <div className="text-[10px] text-faint">R�F�RENCE DE COMMANDE</div>
                  <div className="font-bold text-sm">{commande?.id ? `#${commande.id}` : "—"}</div>
                  <div className="text-[11px] text-muted">Référence de commande</div>
                </div>
              </div>
            </div>
          )}
          {numerique.length > 0 && (
            <div className="card">
              <div className="flex items-center justify-between mb-3">
                <div className="font-bold text-sm flex items-center gap-2"><Smartphone size={16} /> Articles numériques</div>
              </div>
              {numerique.map((a) => (
                <div key={a.id} className="flex gap-3 items-center py-2">
                  <CouvertureLivre graine={a.id} className="w-10 h-14 shrink-0" />
